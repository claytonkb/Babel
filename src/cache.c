...
//        lcl(temp,2) = (rcl(temp,0) - rcl(temp,1));
    ...
    mword *result = _newlfi(this_bvm, 60, 0);
    ...
    lcl(result, 7) = (mword)this_bvm->thread_id;
    lcl(result, 8) = rcl(rci(this_bvm->thread_id,0),0);
    lcl(result, 9) = (mword)this_bvm->steps;
    lcl(result,10) = rcl(rci(this_bvm->steps,0),0);
    lcl(result,11) = (mword)this_bvm->advance_type;
    lcl(result,12) = rcl(rci(this_bvm->advance_type,0),0);
    lcl(result,13) = (mword)this_bvm->mask_table;
    lcl(result,14) = (mword)this_bvm->flags;

    lcl(result,15) = this_bvm->flags->MC_ALLOC_BLOCKING;
    lcl(result,16) = this_bvm->flags->MC_GC_BLOCKING;
    lcl(result,17) = this_bvm->flags->MC_GC_PENDING;
    lcl(result,18) = this_bvm->flags->MC_GC_CONVERT_PENDING_TO_OP_RESTART;
    lcl(result,19) = this_bvm->flags->MC_GC_PNR;
    lcl(result,20) = this_bvm->flags->MC_GC_OP_RESTART;

    lcl(result,21) = this_bvm->flags->INTERP_BOOT_IN_PROGRESS;

    lcl(result,22) = this_bvm->flags->BVM_INSTR_IN_PROGRESS;
    lcl(result,23) = this_bvm->flags->BVM_INCOHERENT;
    lcl(result,24) = this_bvm->flags->BVM_SYMBOLS_DEFINED;
    lcl(result,25) = this_bvm->flags->BVM_CLEAN;
    lcl(result,26) = this_bvm->flags->BVM_INTERP_OP_TRACE;

    lcl(result,27) = this_bvm->flags->BVM_CODE_LIST_EMPTY;
    lcl(result,28) = this_bvm->flags->BVM_RETURN_TOS_ON_EXIT;

    lcl(result,29) = this_bvm->flags->BVM_CACHE_INVALID;
    lcl(result,30) = this_bvm->flags->BVM_CACHE_DIRTY;
    lcl(result,31) = this_bvm->flags->BVM_CACHE_ONLY;
    lcl(result,32) = this_bvm->flags->BVM_CACHE_BLOCKING;

    lcl(result,33) = this_bvm->flags->NO_ASYNC;
    lcl(result,34) = this_bvm->flags->NO_EXCEPT;

    lcl(result,35) = this_bvm->flags->MC_USE_MALLOC;

    lcl(result,36) = (mword)this_bvm->interp;

    lcl(result,37) = (mword)this_bvm->interp->cat_ex;
    lcl(result,38) = (mword)this_bvm->interp->argc;
    lcl(result,39) = (mword)this_bvm->interp->argv;
    lcl(result,40) = (mword)this_bvm->interp->envp;
    lcl(result,41) = (mword)this_bvm->interp->interp_argv;
    lcl(result,42) = (mword)this_bvm->interp->mem;
    lcl(result,43) = (mword)this_bvm->interp->nil;
    lcl(result,44) = (mword)this_bvm->interp->jump_table;
    lcl(result,45) = (mword)this_bvm->interp->empty_string;
    lcl(result,46) = (mword)this_bvm->interp->utc_epoch;
    lcl(result,47) = (mword)this_bvm->interp->srand;
    lcl(result,48) = (mword)this_bvm->interp->null_hash;
    lcl(result,49) = (mword)this_bvm->interp->thread_counter;
    lcl(result,50) = (mword)this_bvm->interp->global_tick_count;

    lcl(result,51) = (mword)this_bvm->interp->mem->primary->base_ptr;
    lcl(result,52) = (mword)this_bvm->interp->mem->primary->alloc_ptr;
    lcl(result,53) = (mword)this_bvm->interp->mem->primary->size;

    lcl(result,54) = (mword)this_bvm->interp->mem->secondary->base_ptr;
    lcl(result,55) = (mword)this_bvm->interp->mem->secondary->alloc_ptr;
    lcl(result,56) = (mword)this_bvm->interp->mem->secondary->size;

    lcl(result,57) = (mword)this_bvm->interp->mem->GC_count;
    lcl(result,58) = (mword)this_bvm->interp->mem->op_restart_alloc_size;
    lcl(result,59) = (mword)this_bvm->interp->mem->last_GC_tick_count;
