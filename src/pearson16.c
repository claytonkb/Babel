...
//    return _pearson_marsaglia(this_bvm, 
//            _newlfi(this_bvm, HASH_SIZE,0), //FIXME: WASTE this needs to be a constant
//            key, 
//            _arlen8(this_bvm, key));

   return _pearson16(this_bvm, 
            BABEL_TAG_ZERO_HASH,
            key, 
            _arlen8(this_bvm, key));
...
//
//
mword *_hash(bvm_cache *this_bvm, mword *key){ // _hash#
...
mword *_pearson16(bvm_cache *this_bvm, mword* sinit, mword *skey, mword strlen){ // _pearson16#
    ...
    char *result = (char*)_newlfi(this_bvm, HASH_SIZE, 0);

    int i;
    char temp;

    for(i=0; i<HASH_SIZE*MWORD_SIZE; i++){ // this enables progressive hashing...
        result[i] = init[i];
    }
    ...
    for(i=0;i<strlen;i++){
        temp = result[0];
        int j;
        for(j=0;j<HASH_BYTE_SIZE-1;j++){
            result[j] = pearson16_byte_perm[ ((j+1) * PERM_TABLE_WIDTH) + ((result[j+1] ^ key[i]) & 0xff) ];
        }        
        result[HASH_BYTE_SIZE-1] = pearson16_byte_perm[ (0 * PERM_TABLE_WIDTH) + ((temp ^ key[i]) & 0xff) ];
    }
...
// Should be a little more collision resistant than pearson16()...
//
mword *_pearson_marsaglia(bvm_cache *this_bvm, mword* sinit, mword *skey, mword strlen){ // _pearson_marsaglia#
    ...
    _marsaglia_fuzz(1); // resets the PRNG...

    for(i=0;i<strlen;i++){
        temp = result[0];
        int j;
        for(j=0;j<HASH_BYTE_SIZE-1;j++){
            result[j] = pearson16_byte_perm[ (((j+1) % HASH_BYTE_SIZE) * PERM_TABLE_WIDTH) + ((result[j+1] ^ key[i]) & 0xff) ];
            result[j] ^= (_marsaglia_fuzz(0) & 0xff);
        }
        result[HASH_BYTE_SIZE-1] = pearson16_byte_perm[ (0 * PERM_TABLE_WIDTH) + ((temp ^ key[i]) & 0xff) ];
        result[HASH_BYTE_SIZE-1] ^= (_marsaglia_fuzz(0) & 0xff);
    }
...
mword _marsaglia_fuzz(mword reset){
    ...
    static mword y=88172645463325252LL;
    if(reset){
        y=88172645463325252LL
    }
    yˆ=(y<<13); yˆ=(y>>7); 
    return (yˆ=(y<<17));
